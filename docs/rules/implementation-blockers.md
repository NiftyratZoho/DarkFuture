...
- Missing information: none for the currently implemented bootlegger side-choice, pass/fail/natural-six/curve-attempt rules. Exact out-of-control skid, spin, and roll templates remain tracked under HZ-004.
- Blocks implementation: none.
- Current code behaviour: bootlegger is available as `Bootlegger Left` or `Bootlegger Right` when the chosen side has a four-lane gap: the vehicle's current two lanes plus two lanes on that side. On a straight, a passed control test reverses facing in place. A failed control test rolls a D6 for the final slide position on the declared side: 1-2 shifts one lane-pair one space ahead, 3-4 shifts two lane-pairs one space ahead, and 5-6 shifts three lane-pairs one space ahead. Final-position rams, crashes, passives, and hazards resolve. A natural 6 before modifiers causes loss of control, applies a 0-damage tyre critical, and resolves an out-of-control forward move instead of the bootlegger. Curve/corner attempts trigger an automatic control test; on success the vehicle moves normally with no bootlegger.
- User notes: Bootlegger diagram shows it only requires a four-lane gap, so players should choose which side to perform it on. If the bootlegger roll fails, the vehicle slides farther than expected and rolls a D6 for final position. Natural 6 before modifiers loses control, causes a 0-damage tyre critical, and resolves as an out-of-control move. Bootlegger must be on a straight; attempting one on a curve/corner causes an automatic control test and, if passed, normal movement only.
...
- Missing information: exact spin-template centre displacement/contact cells if the template moves the vehicle's centre, plus exact roll movement/damage template.
- Blocks implementation: faithful spin centre-contact resolution and exact roll movement template.
- Current code behaviour: control-loss skid rows now roll a one-die skid test. Even skids straight one space and loses 5 mph. Odd drift-skids one space and loses 10 mph; if the skid enters a curve it drifts outward, otherwise a second die chooses direction: odd left, even right. Control-loss `spin` results skid first, then roll the spin-test die; odd uses the blue/anti-clockwise table, even uses the red/clockwise table. The extracted speed loss is applied and final facing is snapped to 45-degree increments relative to the current track heading. A spin facing aligned with the track occupies 2 lanes; every other snapped spin facing occupies a 4-lane footprint used for collisions, passives, smoke blocking, and targeting.
- User notes: A skid test is one die: even skids straight and loses 5 mph; odd drift-skids and loses 10 mph. If it skids onto a curve the drift is outward; otherwise roll one die, even right and odd left. Spin template image supplied. It says to align the template with the front of vehicles which have already spun. Red numbers go clockwise from the vehicle front; blue numbers go counter-clockwise. `12+` is straight ahead. A spin test is a single die: odd is anti-clockwise and even is clockwise. Results are at 45-degree increments to the track: aligned/360, 45, 90, 135, 180, 225, 270, or 315 degrees. A spun vehicle only has a 2-lane or 4-lane occupied footprint; there is no other status.
